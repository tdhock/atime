...
```{r}
atime_write <- function(make.mat.fun, fmt, input.type="data.table"){
  grid.args <- list(
  ...
  if(requireNamespace("readr") && input.type=="data.table")
  ...
  atime::atime(
    ...
    setup={
      ...
      input <- if(input.type=="data.table"){
        data.table(mat)
      }else{
        mat
      }
...
```{r}
best.list <- atime::references_best(write.real.vary.rows)
...
if(require(ggplot2)){
  ...
  gg <- ggplot()+
    ggtitle("Write CSV with 10 random normal real columns, asymptotic complexity")+
    theme_bw()+
...
```{r}
best.list <- atime::references_best(write.chr.vary.rows)
meas.dt <- one.thread(best.list$meas)
ref.dt <- one.thread(best.list$ref)[
  fun.name %in% c("N^2","N") & N >= 1e2]
if(require(ggplot2)){
  hline.df <- with(write.chr.vary.rows, data.frame(
    seconds.limit, unit="seconds"))
  gg <- ggplot()+
    ggtitle("Write CSV from data.table with 10 character columns, asymptotic complexity")+
    theme_bw()+
    facet_grid(unit ~ expr.name, scales="free")+
    geom_hline(aes(
      yintercept=seconds.limit),
...
if(require(ggplot2)){
  hline.df <- with(write.chr.vary.cols, data.frame(
    seconds.limit, unit="seconds"))
  gg <- ggplot()+
    ggtitle("Write CSV from data.table with 10 character rows, asymptotic complexity")+
    theme_bw()+
    facet_grid(unit ~ expr.name, scales="free")+
    geom_hline(aes(
      yintercept=seconds.limit),
...
cache(write.chrmat.vary.rows, atime_write(
  function(N.rows,N.cols=10)chr_mat(N.rows, N.cols),
  "10_chrmat_cols_%s_%d.csv", input.type="matrix"))
...
```{r}
best.list <- atime::references_best(write.chrmat.vary.rows)
meas.dt <- one.thread(best.list$meas)
ref.dt <- one.thread(best.list$ref)[
  fun.name %in% c("N^2","N") & N >= 1e2]
if(require(ggplot2)){
  hline.df <- with(write.chrmat.vary.rows, data.frame(
    seconds.limit, unit="seconds"))
  gg <- ggplot()+
    ggtitle("Write CSV with 10 character columns, asymptotic complexity")+
    theme_bw()+
    facet_grid(unit ~ expr.name, scales="free")+
    geom_hline(aes(
      yintercept=seconds.limit),
...
cache(write.chrmat.vary.cols, atime_write(
  function(N.cols, N.rows=10)chr_mat(N.rows, N.cols),
  "10_chrmat_rows_%s_%d.csv", input.type="matrix"))
...
```{r}
best.list <- atime::references_best(write.chrmat.vary.cols)
meas.dt <- one.thread(best.list$meas)
ref.dt <- one.thread(best.list$ref)[
  fun.name %in% c("N^2","N") & N >= 1e2]
if(require(ggplot2)){
  hline.df <- with(write.chrmat.vary.cols, data.frame(
    seconds.limit, unit="seconds"))
  gg <- ggplot()+
    ggtitle("Write CSV from matrix with 10 character rows, asymptotic complexity")+
    theme_bw()+
    facet_grid(unit ~ expr.name, scales="free")+
    geom_hline(aes(
      yintercept=seconds.limit),
...
### Write CSV with factor data

The code below is for factor data with a constant number of
columns, and a variable number of rows.

```{r}
fac_mat <- function(N.rows, N.cols){
  data.vec <- factor(paste0("'quoted", c(" ", "_"), "data'"))
  matrix(data.vec, N.rows, N.cols)
}
cache(write.fac.vary.rows, atime_write(
  function(N.rows,N.cols=10)fac_mat(N.rows, N.cols),
  "10_fac_cols_%s_%d.csv"))
aplot(write.fac.vary.rows, "Write CSV with 10 factor columns", 1e9, 1e1, "Number of rows", write.colors)
```
...
```{r}
best.list <- atime::references_best(write.fac.vary.rows)
meas.dt <- one.thread(best.list$meas)
ref.dt <- one.thread(best.list$ref)[
  fun.name %in% c("N^2","N") & N >= 1e2]
if(require(ggplot2)){
  hline.df <- with(write.fac.vary.rows, data.frame(
    seconds.limit, unit="seconds"))
  gg <- ggplot()+
    ggtitle("Write CSV with 10 factor columns, asymptotic complexity")+
    theme_bw()+
    facet_grid(unit ~ expr.name, scales="free")+
    geom_hline(aes(
      yintercept=seconds.limit),
...
cache(write.fac.vary.cols, atime_write(
  function(N.cols, N.rows=10)fac_mat(N.rows, N.cols),
...
```{r}
best.list <- atime::references_best(write.fac.vary.cols)
meas.dt <- one.thread(best.list$meas)
ref.dt <- one.thread(best.list$ref)[
  fun.name %in% c("N^2","N") & N >= 1e2]
if(require(ggplot2)){
  hline.df <- with(write.fac.vary.cols, data.frame(
    seconds.limit, unit="seconds"))
  gg <- ggplot()+
...
names(PRGn)[c(
  ...
  some(9,10,11),
  some(14,13,12),
  23,24,25)] <- c(names(read.expr.list),"read.csv(colClasses)","list2DF(scan)")
...
```{r}
best.list <- atime::references_best(read.real.vary.cols)
meas.dt <- one.thread(best.list$meas)
ref.dt <- one.thread(best.list$ref)[
  fun.name %in% c("N^2","N log N", "N") & N >= 1e2]
if(require(ggplot2)){
  hline.df <- with(read.real.vary.cols, data.frame(
    seconds.limit, unit="seconds"))
  gg <- ggplot()+
    ggtitle("Read CSV with 10 real rows, asymptotic complexity")+
    theme_bw()+
    facet_grid(unit ~ expr.name, scales="free")+
    geom_hline(aes(
      yintercept=seconds.limit),
  ...
  ##png("figure-read-real-vary-cols-complexity.png", 12, 6, units="in", res=100)
  print(dl)
  ##dev.off()
}
```
