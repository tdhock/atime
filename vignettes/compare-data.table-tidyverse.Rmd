...
aplot <- function(atime.list, my.title, xmax, max.seconds, xlab, color.vec=NULL){
  out.rds <- paste0(gsub(" ", "_", my.title), ".rds")
  atime.list$hostname <- hostname
  saveRDS(atime.list, out.rds)
...
ref.dt <- one.thread(best.list$ref)[each.sign.rank==1 & N >= 1e2]
if(require(ggplot2)){
  hline.df <- with(write.chr.vary.cols, data.frame(
    seconds.limit, unit="seconds"))
  gg <- ggplot()+
    ggtitle("Write CSV with 10 character rows, asymtotic complexity")+
    theme_bw()+
    facet_grid(unit ~ ., scales="free")+
    geom_hline(aes(
      yintercept=seconds.limit),
...
```{r}
compute.real.vary.rows <- atime_read("10_real_cols_fwrite_*.csv", compute=TRUE)
...
```{r}
compute.chr.vary.rows <- atime_read("10_chr_cols_fwrite_*.csv", compute=TRUE)
...
atime.join.sum <- atime::atime(
  ...
  expr.list=expr.list)
aplot(atime.join.sum, "Join and sum", 1e7, 1e1, "Size of output vector")
```
...
DF <- data.frame(DT)
rownames(DF) <- with(DF, paste0(letter, i))
atime.join.summarize <- atime::atime(
  N=as.integer(10^seq(0, 7, by=0.5)),
  setup={
    select.dt <- data.table(
      letter=sample(LETTERS, N, replace=TRUE),
