...
```{r}
library(data.table)
##Sys.getenv("SLURM_JOB_CPUS_PER_NODE", 1)
...
```{r}
default.expr <- function(default, expr){
  as.list(match.call()[-1])
}
expr.templates <- list(
  "data.table::fwrite"=default.expr(NULL, {
    data.table::setDTthreads(THREADS)
    data.table::fwrite(dt, name.list$fwrite, showProgress = FALSE)
  }),
  "readr::write_csv"=default.expr(readr::readr_threads(), {
    readr::write_csv(
      dt, name.list$write_csv, progress = FALSE, num_threads = THREADS)
  }))
expr.list <- list()
for(fun.name in names(expr.templates)){
  template.list <- expr.templates[[fun.name]]
  threads.list <- list(
    "1"=1, 
    max=max.threads, 
    default=template.list$default)
  for(threads.name in names(threads.list)){
    threads.env <- list(THREADS=threads.list[[threads.name]])
    expr.name <- paste0(fun.name, "\nthreads=", threads.name)
    expr.list[[expr.name]] <- eval(substitute(
      substitute(EXPR, threads.env), 
      list(EXPR=template.list$expr)))
  }
}
