...
`run.atime.versions` in the code above. We do it in the package example code,
in order to run the different versions of the code using `callr::r`,
in a separate R process. This allows us to avoid CRAN warnings about
unexpected files found in the package check directory, by safely
delete/remove the installed packages, after having run the example
code. In the code block below we compute the timings,
