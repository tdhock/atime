...
## Interpolate at seconds.limit

```{r}
interpolation.dt <- with(all.best, references[overall.rank==1, data.table(
  unit="seconds",
  seconds.limit,
  N=approx(empirical, N, seconds.limit)$y
), by=expr.name])
if(require(ggplot2)){
  plot(all.best)+
    geom_point(aes(
