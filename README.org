...
- [[https://docs.google.com/presentation/d/1_aWo2HgM600Mk1nZ4UUv3qV-DzdBmvcEInEU96KqIjY/edit?usp=sharing][Asymptotic benchmarking slides]] for a short talk about atime.
...
Please read and cite the [[https://doi.org/10.32614/RJ-2026-013][atime R Journal (2026) paper]]:
...
You can run =atime_versions()= to check R package performance in every Pull Request.

- [[https://github.com/tdhock/binsegRcpp/blob/poncatime/.github/workflows/performance-tests.yml][Example yml config for binsegRcpp]]: you need to edit the config file for your package.
  - install your package from CRAN, if you want to compare with timings from your current CRAN release.
- Example [[https://github.com/tdhock/binsegRcpp/blob/poncatime/.ci/atime/tests.R][.ci/atime/tests.R]] file: you need to edis this to define performance tests relevant for your package.
- Example output: [[https://github.com/tdhock/binsegRcpp/pull/29#issuecomment-5719035029][PR comment]].
- For more complex example configuration, please read [[https://github.com/Rdatatable/data.table/wiki/Performance-testing][data.table Performance testing wiki page]].
...
[[https://github.com/DavisVaughan/cross][cross]] runs different package versions in different R sessions, so is similar to =atime_versions()=, which runs different package versions in the same R session.
